...
This test file is created to validate the core functionalities of
 the MSSD Course Builder:
- CourseNode: Validates individual nodes within the course tree.
- CourseTree: Verifies that the course tree is built dynamically
 based on student progress.
- read_programs_csv: Checks the program CSV file reading and
 correct parsing.
- read_courses_csv: Ensures courses CSV file reading and correct
 structure of data.
- recommend_courses: Tests that the recommendations are accurate
 based on path interest, courses taken, and the
  student's desired number of courses.

"""

import unittest
from coursebuilder import (read_programs_csv,
                           read_courses_csv,
                           CourseTree)  # Make sure same folder
...
class TestCourseBuilder(unittest.TestCase):
    ...
    def test_read_programs_csv(self):
        ...
        self.assertEqual(programs_data['MSSD']['core_courses'],
                         ['521', '526'])
    ...
    def test_read_courses_csv(self):
        ...
        self.assertEqual(courses_data['673']['course_name'],
                         'Software Engineering')

    def test_course_tree_recommendation(self):
        """
        Test the CourseTree class by simulating a student's course
         progress and path interest.
        """
        ...
        recommended_courses = course_tree.recommend_courses(course_taken,
                                                            path_interest,
                                                            course_to_take)

        # Assert that we receive the correct number of recommendations
        self.assertEqual(len(recommended_courses), course_to_take)
        self.assertIn(655, recommended_courses)
        # Core course 655 should be recommended
        self.assertIn(677, recommended_courses)
        # AI/ML elective 677 should be recommended if not taken

    def test_course_tree_structure(self):
        """
        Test the construction of the course tree structure
        and ensure it's building correctly.
        """
        ...
        self.assertEqual(course_tree.root.course_id, 526)
        # Root should start with the next course in branch
        self.assertEqual(len(course_tree.root.children), 1)
        # Only one branch should continue after the root
