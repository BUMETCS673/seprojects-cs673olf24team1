...
This code defines the structure for dynamically building a course tree
for a Master's in Software Systems Development (MSSD) program,
providing recommendations on course paths based on the student's taken
courses, path interests, and electives.

Key Components:
- CourseNode: Represents individual course nodes.
- CourseTree: Builds the course tree based on taken courses and electives.
- read_programs_csv: Reads a CSV file containing program details
  (core/elective courses, prerequisites).
- read_courses_csv: Reads a CSV file containing detailed course
  descriptions and prerequisites.
- recommend_courses: Suggests a series of recommended courses for a
  student.

Functionality:
1. Builds course sequences dynamically based on core course paths
   (e.g., 521 → 526).
2. Adjusts elective paths based on a student's selected interest and the
   courses they’ve already taken.
3. Implements prerequisite handling for courses like '767', where '677'
   must be taken first.
4. Suggests up to a specified number of courses, based on the number of
   desired courses for the semester.
...
def read_programs_csv(file_path):
    ...
    for _, row in df.iterrows():
        ...
        core_courses = [
            course.strip()
            for course in str(row['core_courses']).split(',')
            if course.strip()
        ]
        elective_courses = [
            course.strip()
            for course in str(row['elective_courses']).split(',')
            if course.strip()
        ]  # Convert row['elective_courses'] to a string before split()
...
class CourseNode:
    ...
    def __repr__(self):
        return (f"{self.course_id} -> "
                f"{[child.course_id for child in self.children]}")
...
class CourseTree:
    ...
    def add_branch(self, branch, course_taken, skip_courses=None):
        """
        Adds a branch to the tree, skipping courses already taken and any
        optional skip courses.
        """
        # If the root is not set, initialize it with the first course
        if not self.root:
            self.root = CourseNode(branch[0])  # Initialize the root
            current_node = self.root
        else:
            # If the root is set, check if the first course is already the root
            if self.root.course_id == branch[0]:
                current_node = self.root
            else:
                # If not, find it or add it
        ...
        for course_id in branch[1:]:
            if (course_id in course_taken or
                    (skip_courses and course_id in skip_courses)):
                # Skip if already taken or need to skip
                continue

            # Check if the node already exists
            child_node = next(
                (child for child in current_node.children
                 if child.course_id == course_id),
                None,
            )
            if not child_node:
            ...
            current_node = child_node  # Move to next course

    def convert_course_taken_to_int(self, course_taken):
        """
        Convert all course IDs in the course_taken list from
        strings to integers.
        """
    ...
    def build_mssd_tree(self, course_taken, path_interest):
        """
        Builds the MSSD course tree dynamically based on the courses already
        taken and path interest.
        ...
        - When adding other MS degrees in the CS degree, don't forget to
          change the exceptions and course numbers.
        - For certification, need to make a new type of tree (but simpler,
          use the earlier branch method).
        """
        ...
        # Step 1: Choose the appropriate core based on path_interest
        # and course_taken
        core_options = {
            'web development': [[622, 655, 673], [673, 622, 655],
                                [622, 673, 655]],
            ...
            'app development': [[622, 655, 673], [673, 622, 655],
                                [622, 673, 655]],
            'ai/ml': [[622, 655, 682], [682, 622, 655], [622, 682, 655]],
            'data science': [[622, 655, 682], [682, 622, 655],
                             [622, 682, 655]],
        }
        ...
        # FIXED: now can detect if took 673 or 682, and if have not can
        # detect based on path_interest
        if 673 in course_taken or 682 in course_taken:
            # Both 673 and 682 are in course_taken. Skipping both.
        ...
        # Step 2: Add electives based on path_interest, but only up
        # to 3 total electives
        elective_paths = {
            'web development': [[601, 602, 701]],
            'ai/ml': [[664, 677, 767], [677, 644, 767], [677, 767, 664]],
            'data science': [[669, 677, 767], [677, 669, 767],
                             [677, 767, 669]],
            'secure software development': [
                [633, 634, 763],
                [634, 633, 763],
            ...
            'app development': [[683, 601, 602], [601, 683, 602],
                                [601, 602, 683]],
        }

        # Detect electives already taken from course_taken
        electives_taken = [course for course in course_taken
                           if course in elective_list]
        ...
        if remaining_electives > 0:
            ...
            chosen_sequence = (
                random.choice(elective_paths.get(path_interest, [])))
            # print(f"Chosen elective sequence: {chosen_sequence}")

            electives_to_add = [
                course for course in chosen_sequence if
                course not in course_taken
            ]

            # Apply specific prerequisite rules
            if 677 not in course_taken and 677 in electives_to_add:
                electives_to_add = [
                    course for course in electives_to_add if course != 767
                ]

            if 601 not in course_taken and 601 in electives_to_add:
                electives_to_add = [
                    course for course in electives_to_add if course != 602
                ]

            # print(f"Electives to add (after prerequisite checks):
            # {electives_to_add}")

            # Add electives to tree, ensuring we fill up to
            # 3 electives if needed
            for elective in electives_to_add[:remaining_electives]:
                # print(elective)
                self.add_branch([elective], course_taken)
                # print(branches)

    def recommend_courses(self, course_taken, path_interest, course_to_take):
        """
        Recommend the next set of courses based on the student's taken courses,
        elective interest, and how many courses they want to take in the next
        semester.
        """
        ...
        # Step 2: Traverse the tree and recommend up to
        # 'course_to_take' courses
        def traverse_tree(node):
            if (
                    node.course_id not in course_taken
                    and node.course_id not in self.recommended_courses
            ):
        ...
        if len(self.recommended_courses) < course_to_take:
            self.recommended_courses.extend(
                [''] * (course_to_take - len(self.recommended_courses))
            )
...
def recommend_courses(courses_taken, path_interest, num_courses_to_take):
    tree = CourseTree()
    return tree.recommend_courses(courses_taken, path_interest,
                                  num_courses_to_take)
